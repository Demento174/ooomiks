<?php
namespace Classes\Import\Suppliers;



use Classes\PostsAndTax\PostProduct;
use Classes\PostsAndTax\TaxCat;
use \Classes\XML\XmlStringStreamerWrapper;
...
class Matrix
{
    ...
    public function handler_categories(bool $debug_mode=false,$count_rows_debug_mode=2)
    {
        ...
        return \Classes\XML\XmlStringStreamerWrapper::init_parse($this->file,
    ...
    public function handler_products(bool $debug_mode=false,$count_rows_debug_mode=2)
    {
            ...
            function ($node) use ($debug_mode,$count_rows_debug_mode)
        {
            ...
            foreach ($node->offers->offer as $offer)
            {
                ...
                if(\Classes\PostsAndTax\PostProduct::existence_product_by_article_matrix($article))
                {
                    ...
                    $product->update(
                        ...
                        $weight,
                        $attributes,
                        [\Classes\PostsAndTax\PostProduct::$selectors['articles']['matrix']=>$article]
                    );
                    $response[$i]['action'] = 'Update';
                }else
                    {
