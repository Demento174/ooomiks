	...
	private function get_select_html($opt_name, $feed_id = '1', $otions_arr = array()) {
		...
		$res = '<select name="'.$opt_name.'" id="'.$opt_name.'">
					...
					if (isset($otions_arr['brands'])) {
						...
						if (is_plugin_active('yith-woocommerce-brands-add-on/init.php') || is_plugin_active('perfect-woocommerce-brands/main.php') || class_exists('Perfect_Woocommerce_Brands')) {
							$res .= '<option value="yith_woocommerce_brands_add_on" '. selected($opt_value, 'yith_woocommerce_brands_add_on', false).'>'. __('Substitute from', 'yfym'). ' YITH WooCommerce Brands Add-On</option>';
						}
					}
