...
trait YFYM_T_Variable_Get_Name {
	public function get_name($tag_name = 'name', $result_xml = '') {
		...
		$result_yml_name = $product->get_title(); // htmlspecialchars($cur_post->post_title); // название товара
		$result_yml_name = apply_filters('y4ym_f_simple_tag_value_name', $result_yml_name, array('product' => $product, 'offer' => $offer), $this->get_feed_id());
		...
		$result_yml_name = apply_filters('yfym_variable_change_name', $result_yml_name, $product->get_id(), $product, $offer, $this->get_feed_id());
		$result_yml_name = apply_filters('y4ym_f_variable_tag_value_name', $result_yml_name, array('product' => $product, 'offer' => $offer), $this->get_feed_id());
		...
		if (empty($no_group_id_arr)) {
			...
			if (in_array($сur_сategory_id, $no_group_id_arr)) {
				...
				if ($param_at_name == '') {
					$this->add_skip_reason(array('offer_id' => $offer->get_id(), 'reason' => __('There are no variable attributes to create a unique product name', 'yfym'), 'post_id' => $offer->get_id(), 'file' => 'trait-yfym-t-variable-get-name.php', 'line' => __LINE__)); return '';
				}
...
/* ! потенциально заменить на mb_strimwidth() */
				...
				$result_yml_name_itog = apply_filters('yfym_name_no_groupid_filter', $result_yml_name_itog, $result_yml_name, $product, $so, $sz, $sd, $sr, $this->get_feed_id());
			} else {
		...
		$result_xml = apply_filters('y4ym_f_variable_tag_name', $result_xml, array('product' => $product, 'offer' => $offer), $this->get_feed_id());
		return $result_xml;	
	}
}
?>